...
if cashInGold != None and cashInGold > 0:
    print_colorvars('{} had maar {} goud op zak, dus was wel geinterreseerd in meer.',
                    [mainCharacter['name'], cashInGold])
...
if len(friends) > 0:
    ...
    print_colorvars('{} besloot het met {} te delen, en {} mee op avontuur.',
                    [mainCharacter['name'], shareFriendText, comigFriendText])
        ...
        if cashInGold == None:
            ...
            if totalCost < cashInGold:
                if len(investors) == 0:
                    ...
                    investorsText = ifOne(amountOfInvestorsCommingAlong, 'investeerder aan sluit',
                                          'investerders aan sluiten', '1')

                    print_colorvars('Er werd gezocht naar investeerders en vonden er {} waarvan er {}.',
                                    [len(interestingInvestors), investorsText])
                    print_colorvars(
                        'Het is gebruikelijk dat een investeerder zijn eigen paard, tent en uitrusting krijgt, dat maakt het totaal nu {} en {}',
                        [horseText, tentText])

            if totalCost > cashInGold:
                startPrepwork = False
                print_colorvars('De totale kosten tot nu toe {} goud', [totalCost])
                print_colorvars('Zoveel heeft de groep niet, ze hebben maar {} goud, dus ze bleven maar thuis.',
                                [cashInGold])
            else:
                print_colorvars('De totale kosten tot nu toe {} goud, gelukkig heeft de groep {} goud te besteden.',
                                [totalCost, cashInGold])
...
if startPrepwork:
    ...
    if JOURNEY_IN_DAYS == 0:
        if len(mainCharacter['name']) == 0:
            ...
            print_colorvars(
                'De schatkaart is nog niet voldoende bestudeerd om te weten hoe de reis gaat verlopen, maar wat maakt het uit, dacht {}, gewoon gaan!',
                [mainCharacter['name']])
        ...
        if journeyFoodCost != None and journeyFoodCost > 0:
            ...
            print_colorvars('Het eten voor {} en {} tijdens de reis gaat {} goud kosten.',
                            [adventurerText, horseText, journeyFoodCost])
        ...
        if len(investors) > 0:
            ...
            if nightsInInn > 0:
                ...
                print_colorvars(
                    'Er werd uitgerekend dat met het overgebleven {} goud er {} van de {} in een herberg overnacht kon worden.',
                    [leftoverGold, nightsInInn, nightsText])
        ...
        if cashInGold != None and totalCost > cashInGold:
            ...
            print_colorvars('Het {}-tal kan helaas niet vertrekken, voordat ze weten of het wel te betalen is.',
                            [len(party)])
        else:
...
if adventureStarted:
    ...
    if len(mainCharacter['name']) == 0:
        ...
        print_colorvars(
            'Een reis zonder het bestuderen van de kaart bleek toch geen goed idee, ineens zag {} daar weer het beginpunt en kon er opnieuw begonnen worden.',
            [mainCharacter['name']])
    elif journeyFoodCost == None or journeyFoodCost == 0:
        print_colorvars('Reizen zonder eten doet {} geen goed en hij keerde al snel terug naar huis.',
                        [mainCharacter['name']])
    elif len(party) == 1:
        print_colorvars(
            'Al gouw kwam {} er achter dat een avontuur in je eentje niet leuk is en keerde terug naar huis.',
            [mainCharacter['name']])
    elif rentStuff == False:
        adventurersText = ifOne(len(party), 'avonturier', 'avonturiers')
        horseText = ifOne(horses + 1, 'paard', 'paarden', '1')
        print_colorvars(
            'Op de tweede dag, kwam de groep van {} er achter dat een avontuur zonder tenten en {} je niet een prettige reis hebt, dus gingen weer terug.',
            [adventurersText, horseText])
    elif len(adventurerGear) == 0:
        adventurersText = ifOne(len(party), 'avonturier', 'avonturiers')
        print_colorvars(
            'Bij het eerste de best obstakel, kwam de groep van {} er achter dat een avontuur zonder uitrusting niet heel handig was en ze keerde terug naar huis.',
            [adventurersText])
    elif nightsInInn == 0:
        investorsAdventuring = getAdventuringInvestors(investors)
        investorsText = ifOne(len(investorsAdventuring), 'investeerder', 'investeerders', '')
        print_colorvars(
            'Het blijkt echter geen goed idee om met de {} iedere nacht in een tent slapen, dus ze keren terug naar huis om een nieuw plan te maken.',
            [investorsText])
    else:
        print_colorvars('De reis voorloop spoedig voor het {}-tal en ze komen op tijd aan bij de bestemming.',
                        [len(party)])

        if len(treasure) == 0:
            print_colorvars(
                'Helaas blijkt de schat al te zijn opgegraven door iemand anders en keren ze met lege handen terug naar huis.')
        else:
            ...
            print_colorvars('Iedereen kreeg eerst zijn geinvesteerde goud terug en zo bleef er nog {} goud winst over.',
                            [profitGold])
            ...
            if profitGold <= 0:
                ...
                if len(investors) > 0 and investorsCuts != None:
                    print_colorvars(
                        'Als investeerder krijg je altijd eerst geld voordat de rest eerlijk verdeeld wordt, dat ziet er zo uit:')

                    index = 0
                    for investor in getInterestingInvestors(investors):
                        print_colorvars('{} krijgt {}{}, dat is {} goud.',
                                        [investor['name'], investor['profitReturn'], '%', investorsCuts[index]])
                        index += 1
                elif len(investors) > 0 and investorsCuts == None:
                    investorsText = ifOne(len(investors), 'investeerder moest', 'investeerders moesten', '')
                    print_colorvars('De {} eerst hun geld krijgen voordat er verder gerekend kan worden.',
                                    [investorsText])
                else:
                ...
                if adventurerGoldCut == None:
                    ...
                    adventurersText = ifOne(len(party), 'persoon die op avontuur is geweest',
                                            'personen die op avontuur zijn geweest', 'ene')
                    print_colorvars('Nu alleen noog het overgebleven {} goud verdelen over die {}.',
                                    [leftoverGold, adventurersText])
                    ...
                    if earnings != None:
                        print_colorvars(
                            'Dat betekend dat {} eerst {} goud hand en nu {} goud heeft, wat {} goud winst is.',
                            [earnings[0]['name'], earnings[0]['start'], earnings[0]['end'], personalProfit])
                    else:
                    ...
                    if len(investors) > 0:
                        print_colorvars(
                            'Na het uitbetalen komt er op neer dat er {} goud winst aan iedere van de {} avonturiers gegeven kan worden.',
                            [adventurerGoldCut, len(party)])
                    else:
                        print_colorvars('Dat krijgt ieder van de {} avonturiers {} goud.',
                                        [adventurerGoldCut, len(party)])
                    ...
                    if earnings == None:
                        ...
                        if len(friends) > 0:
                            if len(friends) == 1:
                                print_colorvars(
                                    'De vriend van {0} besluit om {0} 10 goud van zijn verdiensten te geven.',
                                    [mainCharacter['name']])
                            else:
                                print_colorvars(
                                    'De vrienden van {0} besluiten om 10 goud van hun verdiensten af te staan aan {0}',
                                    [mainCharacter['name']])
                        ...
                        for earning in earnings:
                            ...
                            print_colorvars('{} had eerst {} goud en heeft nu {} goud, dat betekend {} goud winst.',
                                            [earning['name'], earning['start'], earning['end'], personalProfit])
